...
`kind` values include `image`, `audio`, `document`, `video`, `youtube_video`, `stl`, `live_stream`, and `link`. Local media paths are rewritten from protected Web Chat `/files/...` URLs to short-lived signed `/api/agodesk/media/<bucket>/<file>` URLs. The server only serves explicit media buckets (`images`, `generated_images`, `generated_videos`, `audio`, `documents`, and `downloads`), rejects traversal, and rejects missing, expired, or tampered signatures. Clients must use `path` and `preview_url` exactly as provided after JSON decoding, including query parameters. There is currently no WebSocket command to refresh a media URL after a `401`; a new server-emitted `chat.media` event is required. YouTube payloads use `url`, `embed_url`, `video_id`, `title`, `provider:"youtube"`, and optional `start_seconds`; no media asset fetch is needed.

When extracting `/files/...` references from assistant or tool prose, AuraGo stops at Markdown/JSON delimiters, `&`, `?`, and `#` before rewriting and signing. Structured media payloads retain their query parameters (such as `inline=1`), and percent-encoded filename characters remain encoded. Clients must not strip or repair filename characters on an already signed URL.
