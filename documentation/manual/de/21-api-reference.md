...
- `chat.media` wird gesendet, wenn `chat.media_events` ausgehandelt ist, und transportiert Nicht-TTS-Bilder, Audio/Musik, Dokumente, Videos, STL, Links und YouTube-Embeds. Geschützte `/files/...` Assets werden auf kurzlebig signierte `/api/agodesk/media/...` URLs umgeschrieben; Clients müssen die gelieferten Query-Parameter beibehalten und Media-Metadaten nach `401` neu laden.
