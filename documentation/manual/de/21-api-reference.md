...
`/api/agodesk/ws` nutzt das AgoDesk-JSON-Envelope-Protokoll. Produktionsclients koppeln sich mit `session.start`. Lokale Entwicklung benötigt zusätzlich ein mindestens 32 Zeichen langes `AURAGO_AGODESK_DEV_TOKEN` und den passenden Header `X-AuraGo-Agodesk-Dev-Token` zusammen mit `?insecure_loopback=1`. Nach `session.accepted` speichert AgoDesk `advertised_capabilities` und nutzt die akzeptierte `session_id` als Transport-Session.

Clients mit ausgehandelter Capability `local.agent` können `local.agent.remote_tool` für begrenzte Memory-/Abfrage-Adapter, `local.agent.llm` für einen direkten Provider-Aufruf mit Vault-geschützten Zugangsdaten und `local.agent.turn` zur Fire-and-forget-Synchronisierung von Verlauf und Aktivität nutzen. Nur `local.agent.handoff` startet dabei den vollständigen AuraGo-Agentenloop. LLM-Requests benötigen einen sekundengenauen RFC3339-`client_timestamp`; ohne `provider_id` wird der aktive Hauptprovider verwendet. Vorherige Assistant-Tool-Calls akzeptieren sowohl die flache AuraGo-Form als auch OpenAI-Wire-JSON-Strings in `function.arguments`. LLM-Ergebnisse spiegeln die `request_id` und nutzen `success`, `message`, `error_code` und `error_message`; Remote-Tool-Fehler behalten ihren passenden `.result`-Envelope.
...
Startet den asynchronen Abruf und Image-Build des konfigurierten Git-Refs. Der bisherige Container wird erst nach einem erfolgreichen Build ersetzt.
