...
- Audio, RTP packets, complete SIP headers, authentication material, and raw
  transcripts are never logged or stored. Call history contains only IDs,
  direction, normalized peer, timestamps, state, end reason, backend, and an
  optional chat-session link.
- Transcripts are transient by default. Enabling `persist_transcripts` retains
  only the existing chat session; it does not create a call recording.
...
empty list means no native tools.
...
`aurago_execute`, `aurago_cancel_current_task`, and `aurago_end_call`.
