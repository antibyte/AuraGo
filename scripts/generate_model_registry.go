...
// Usage:
//
//	go run scripts/generate_model_registry.go --check
//	go run scripts/generate_model_registry.go --write
//
// This will:
...
func main() {
	check := flag.Bool("check", false, "compare the generated registry without writing")
	write := flag.Bool("write", false, "write the generated registry")
	flag.Parse()
	if *check == *write {
	...
	// Generate Go file. The source hash is stable and makes the exact upstream
	// payload auditable without embedding a timestamp that would drift on every
	// --check invocation.
...
const ModelsDevSourceURL = "https://models.dev/api.json"
const ModelsDevSourceSHA256 = "` + sourceHash + `"
...
`, key, m.ID, m.Name, m.Provider, m.ContextWindow, m.MaxOutputTokens,
	...
	if *check {
		...
		if err != nil || !bytes.Equal(current, formatted) {
			fmt.Fprintf(os.Stderr, "models.dev registry is out of date: %s\n", outFile)
			os.Exit(1)
		}
		fmt.Printf("models.dev registry is current (sha256 %s)\n", sourceHash)
		return
	}
