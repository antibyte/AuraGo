...
async function emitBundle(bundle, content) {
  ...
  if (checkOnly) {
    ...
    if (!existing || !existing.equals(expected)) {
      console.error(`Outdated ${toPosixPath(bundle.out)}; run npm run build:ui`);
      process.exitCode = 1;
    } else {
      console.log(`Checked ${toPosixPath(bundle.out)} (${bundle.parts.length} parts)`);
    }
    return;
  }
...
async function buildJSBundle(bundle) {
  ...
  await emitBundle(bundle, normalizeBundleContent(header + sections.join('')) + '\n');
}
