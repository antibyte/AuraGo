...
- System-managed Game Maker Agent Skills must scan clean and match the embedded package hash. Startup self-heals the curated `SKILL.md` files by overwriting local drift with the bundled version and rescanning; warning, blocked, missing, or post-install hash-mismatched packages still block new jobs.
