...
- Helper and KG JSON completions request structured output only for supported routes, strip fenced/thinking wrappers defensively, reject empty/invalid/truncated (`finish_reason=length`) output before caching, and retry once with a smaller unit of work. File KG replacement is all-or-nothing across extraction segments.
...
- Channel question detection includes open channel questions and radio checks without punctuation or assistant addressing. Reception replies confirm only arrival at this node, without web search or claims about audio, other receivers, signal quality or a direct RF path. Statements and other bots' answers still receive no reply.
...
- Preview iframes omit `allow-same-origin`, use short-lived project/job-bound tokens, restrictive CSP including document `sandbox allow-scripts` (so a top-level tab cannot ride the admin session), external-connect blocking, and a source/channel-validated bridge for bounded diagnostics and finite test inputs.
...
This project is indexed by GitNexus as **AuraGo** (74286 symbols, 316826 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
