...
- HA Switchboard is planned in `documentation/ha-switchboard-plan.md`, not implemented. Its supplied visual reference requires a realistic walnut cabinet with silver metal levers, one per selected Home Assistant switch; retain the existing HA integration and desktop shell.
