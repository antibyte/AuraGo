...
- Prompt Markdown without frontmatter remains a compatible plain source. A source that starts frontmatter but cannot parse is rejected; a malformed disk override falls back to its valid embedded source and never becomes unconditional core or tool-guide content.
...
- `RunConfig.TrustedPromptAddenda` is internal trusted execution context only. A2A and co-agent contracts are fitted into the generated prompt so delegated requests have one leading system message; user and external content must remain in user messages or isolated context blocks.
- Runtime persona, emotion, and inner-voice values are untrusted advisory data isolated with `<external_data>` and may affect tone only. The lightweight Looper omits memory/RAG/personality but still uses route-aware fitting, atomic history trimming, final validation, and prompt logging on every LLM round.
