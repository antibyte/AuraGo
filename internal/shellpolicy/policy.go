...
var (
	directDestructiveShellPatterns = []pattern{
	...
	downloadExecutePatterns = []pattern{
	...
	sensitiveEnvReadPatterns = []pattern{
	...
	controlOperatorPattern = regexp.MustCompile(`(&&|\|\||;|\r|\n)`)
)
...
// ValidateCommand rejects shell commands that fall into high-risk classes.
...
// BlockedReason returns the first policy reason for a blocked command.
func BlockedReason(command string) string {
	...
	if reason := match(normalized, interpreterBypassPatterns); reason != "" {
		return reason
	}
	if reason := match(normalized, downloadExecutePatterns); reason != "" {
		return reason
	}
	if reason := match(normalized, sensitiveEnvReadPatterns); reason != "" {
		return reason
	}
	if reason := match(normalized, directDestructiveShellPatterns); reason != "" {
		if controlOperatorPattern.MatchString(normalized) {
...
func match(command string, patterns []pattern) string {
