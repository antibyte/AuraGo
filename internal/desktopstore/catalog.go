...
func DefaultCatalog() []CatalogEntry {
	return withCatalogDefaults([]CatalogEntry{
		...
		},
		{
			...
			Icon:        "server",
			LogoSlug:    "arcane",
			LogoURL:     logoURL("arcane"),
			PrimaryPort: PortSpec{ID: "web", Name: "Web UI", ContainerPort: 3552, Protocol: "tcp"},
			Volumes: []VolumeTemplate{
				{NameSuffix: "data", ContainerPath: "/app/data"},
			},
			Env: []string{
			...
			GeneratedSecrets: []GeneratedSecret{
				{Key: "encryption_key", Env: "ENCRYPTION_KEY", Label: "Encryption key"},
				{Key: "jwt_secret", Env: "JWT_SECRET", Label: "JWT secret"},
			},
			Metadata: map[string]string{
				"open_external": "true",
			...
			Companions: []CompanionTemplate{
				{
					ID:          "socket-proxy",
					Name:        "Arcane Docker Socket Proxy",
					Image:       "tecnativa/docker-socket-proxy:latest",
					NetworkMode: "aurago-store-arcane-net",
					Env: []string{
						"EVENTS=1",
						"PING=1",
						"VERSION=1",
