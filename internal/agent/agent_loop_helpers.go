...
func channelAdaptiveAlwaysInclude(runCfg RunConfig, alwaysInclude []string, ff ToolFeatureFlags) []string {
	...
	if ff.VirtualDesktopEnabled {
		out = append(out, "desktop_notes", "virtual_desktop_files", "virtual_desktop_app_install", "virtual_desktop_apps", "virtual_desktop_widgets")
	}
...
func isToolError(resultContent string) bool {
	return classifyLegacyToolResult(resultContent).IsError()
}
