...
type PersistRecoveryParams struct {
	...
	I18nKey          string // i18n key for localized broker notification (e.g. "backend.stream_error_recovery_raw_code")
	// Optional overrides
...
func (rss *RecoverySessionState) PersistRecoveryMessages(
	...
	// Send broker event — use i18n-translated message for UI when key is provided
	eventType := params.BrokerEventType
	if eventType == "" {
		eventType = "error_recovery"
	}
	if params.I18nKey != "" && rss.cfg != nil {
		rss.broker.Send(eventType, i18n.T(rss.cfg.Server.UILanguage, params.I18nKey))
	} else {
...
func FormatDesktopAnnouncementCompletionFeedback(useNativeFunctions bool) string {
	if useNativeFunctions {
		return "ERROR: You marked the Virtual Desktop task as complete with <done/>, but no desktop tool call ran after your action promise. You MUST call the appropriate native tool now: use virtual_desktop for files/apps/open_in_app, office_document for Writer documents, or office_workbook for spreadsheets. Do not ask the user to repeat the task."
	}
