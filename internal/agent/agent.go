...
func dispatchInner(ctx context.Context, tc ToolCall, dc *DispatchContext) (output string) {
	...
	if blocked := roleToolRestriction(tc, dc); blocked != "" {
		return blocked
	}
	...
	if reason := toolRuntimeUnavailable(dc, tc.Action); reason != "" {
		return toolSetupRequiredOutput(reason)
	}
	...
	homepageSubOps := map[string]bool{
		...
		"deploy_netlify": true, "deploy_here_now": true, "deploy_vercel": true, "publish_local": true, "deploy": true,
