	...
	return append(tools,
		...
		tool("game_maker_asset",
			...
			schema(map[string]interface{}{
				...
				"view":             map[string]interface{}{"type": "string", "enum": []string{"side", "top", "board", "3d", "isometric"}},
				"asset_ids":        map[string]interface{}{"type": "array", "items": map[string]interface{}{"type": "string"}, "minItems": 1, "maxItems": 64, "description": "Required exact IDs for model3d, atlas sprite schema 2, effect or audio import_pack; omit only for legacy whole-sheet sprite packs"},
...
func gameMakerSceneSchema() map[string]interface{} {
	...
	return schema(map[string]interface{}{
		"schema_version": prop("integer", "Scene schema version: 1 for existing scenes; 2 for isometric grid coordinates and elevation levels"),
		"projection": schema(map[string]interface{}{
			"kind":        map[string]interface{}{"type": "string", "enum": []string{"isometric"}},
			"tile_width":  map[string]interface{}{"type": "integer", "enum": []int{128}},
			"tile_height": map[string]interface{}{"type": "integer", "enum": []int{64}},
			"height_step": map[string]interface{}{"type": "integer", "enum": []int{32}},
		}, "kind", "tile_width", "tile_height", "height_step"),
...
func gameMakerSceneGenerateSchema() map[string]interface{} {
	...
	return schema(map[string]interface{}{
		...
		"tile_roles":      schema(map[string]interface{}{"edge": prop("string", "Accepted isometric edge role"), "outer_corner": prop("string", "Accepted outer corner role"), "inner_corner": prop("string", "Accepted inner corner role"), "stairs": prop("string", "Accepted one-step stair role")}),
		"connect_heights": prop("boolean", "Isometric grid only: create declared stairs to adjacent floors one height step above, preserving pinned nodes"),
