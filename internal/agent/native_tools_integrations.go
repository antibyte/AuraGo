...
func appendInvasionFocusedSchemas(toolSchemas []openai.Tool) []openai.Tool {
	toolSchemas = append(toolSchemas,
		tool("invasion_nests", "Manage Invasion Control nests and assignments.", schema(map[string]interface{}{
			"operation": operationProperty("Nest operation.", []string{"list_nests", "list_eggs", "nest_status", "assign_egg", "hatch_egg", "stop_egg", "egg_status"}),
		...
		tool("invasion_tasks", "Send and inspect Invasion Control tasks. Egg names are not tool names.", schema(map[string]interface{}{
			"operation": operationProperty("Task operation.", []string{"send_task", "task_status", "get_result", "list_egg_messages", "ack_egg_message", "send_host_message", "send_secret"}),
			"nest_id":   prop("string", "Nest ID for send_task or send_secret."),
			"egg_id":    prop("string", "Egg ID."),
			"egg_name":  prop("string", "Egg name; not a tool name."),
			"task_id":   prop("string", "Task ID."),
			"id":        prop("string", "Message or artifact ID alias."),
			...
			"priority":  prop("integer", "Host message priority."),
		}, "operation")),
		tool("invasion_artifacts", "Read and manage Invasion Control task artifacts.", schema(map[string]interface{}{
			"operation":   operationProperty("Artifact operation.", []string{"list_artifacts", "get_artifact", "read_artifact", "upload_artifact"}),
			"task_id":     prop("string", "Task ID."),
			"artifact_id": prop("string", "Artifact ID."),
			"id":          prop("string", "Artifact ID alias."),
			"file_path":   prop("string", "Artifact file path."),
			"content":     prop("string", "Inline artifact content for upload_artifact."),
			"mime_type":   prop("string", "Artifact MIME type for upload_artifact."),
		}, "operation")),
	)
	return toolSchemas
}
...
func appendRemoteControlFocusedSchemas(toolSchemas []openai.Tool) []openai.Tool {
	...
	toolSchemas = append(toolSchemas,
		...
		tool("remote_control_shell", "Execute shell commands and persistent shell sessions on connected remote devices.", schema(map[string]interface{}{
			"operation":       operationProperty("Shell operation.", []string{"execute_command", "shell_session_start", "shell_session_read", "shell_session_input", "shell_session_stop", "shell_session_list"}),
			"device_id":       deviceProps["device_id"],
			"device_name":     deviceProps["device_name"],
			"command":         prop("string", "Shell command to execute."),
			"cwd_id":          prop("string", "AgoDesk working-directory root id for shell_session_start."),
			"initial_wait_ms": prop("integer", "Initial read wait after shell_session_start. This is not the session lifetime."),
			"session_id":      prop("string", "Shell session id returned by shell_session_start."),
			"offset":          prop("integer", "Output offset for shell_session_read."),
			"limit":           prop("integer", "Maximum bytes or characters to read from the shell session."),
			"wait_ms":         prop("integer", "Long-poll wait for shell_session_read."),
			"input":           prop("string", "Input to send to shell_session_input."),
		}, "operation")),
		tool("remote_control_files", "Read, write, edit, and search files on connected remote devices.", schema(map[string]interface{}{
			"operation":       operationProperty("File operation.", []string{"read_file", "write_file", "file_patch", "list_files", "edit_file", "json_edit", "yaml_edit", "xml_edit", "file_search", "file_read_advanced"}),
			"device_id":       deviceProps["device_id"],
			"device_name":     deviceProps["device_name"],
			"path":            prop("string", "Remote file or directory path."),
			"root_id":         prop("string", "Agodesk root id for file access."),
			"content":         prop("string", "File content or inserted text."),
			"expected_sha256": prop("string", "Required current SHA-256 of the target file for file_patch."),
			"dry_run":         prop("boolean", "Dry-run file_patch first. Defaults to true."),
			"patches": map[string]interface{}{
				"type":        "array",
				"description": "file_patch replacements as {old_text,new_text,expected_occurrences}.",
				"items": map[string]interface{}{
					"type": "object",
					"properties": map[string]interface{}{
						"old_text":             prop("string", "Exact text to replace."),
						"new_text":             prop("string", "Replacement text."),
						"expected_occurrences": prop("integer", "Expected number of matches."),
					},
				},
			},
	...
	if ff.ChromecastEnabled {
		tools = append(tools, tool("chromecast",
			...
			schema(map[string]interface{}{
				...
				"url":          prop("string", "Direct HTTP(S) media URL to cast (for 'play'). Public URLs are SSRF-protected; private LAN media requires chromecast.media_host_allowlist unless it is AuraGo-generated /tts/ or /cast-media/."),
				"local_path":   prop("string", "Local workspace audio/video/image file to cast (for 'play'). AuraGo publishes it on /cast-media/ automatically, e.g. 'workdir/song.mp3' or 'workdir/clip.mp4'."),
				"content_type": prop("string", "MIME type of the media (for 'play', e.g. 'audio/mpeg', 'video/mp4', 'video/webm'). Direct video URLs should specify this. Default: 'audio/mpeg'."),
	...
	if ff.TTSEnabled {
		tools = append(tools, tool("tts",
			"Convert text to speech (TTS). The generated audio will AUTOMATICALLY be sent to the user and played in the chat UI! "+
				"Supports Google, ElevenLabs, MiniMax, Mistral, Piper, and Supertonic TTS providers. "+
	...
	if ff.FrigateEnabled {
		tools = append(tools, tool("frigate",
			...
			schema(map[string]interface{}{
				...
				"reviewed":     prop("boolean", "Filter reviews by reviewed state"),
				"severity":     prop("string", "Review severity filter, for example alert or detection"),
				"start_time":   prop("string", "Recording clip start time (Unix timestamp)"),
				"end_time":     prop("string", "Recording clip end time"),
				"playback":     prop("string", "Legacy recording playback hint; ignored by current direct clip endpoint"),
	...
	if ff.CydEnabled {
		tools = append(tools, tool("cyd_display",
			...
			schema(map[string]interface{}{
				...
				"page":       prop("string", "status, home, load, work, host, alerts, or mesh"),
		...
		tools = append(tools, tool("huggingface",
			...
			schema(map[string]interface{}{
				"operation": operationProperty("Hugging Face operation.", []string{
					...
					"dataset_splits", "dataset_rows", "dataset_search", "dataset_filter", "dataset_parquet", "dataset_statistics",
					"daily_papers", "search_papers", "get_paper", "paper_links",
					"jobs_list", "job_get", "job_logs", "job_run_python", "job_run_uv_script", "job_run_container", "job_cancel",
					"create_repo", "upload_file", "create_discussion", "comment_discussion", "delete_repo",
				}),
				"query":           prop("string", "Search query for Hub or Papers."),
				"limit":           prop("integer", "Maximum number of results."),
				"repo_type":       prop("string", "Repository type: model, dataset, or space."),
				"repo_id":         prop("string", "Hugging Face repository ID such as org/name."),
				"name":            prop("string", "Repository name alias."),
				"revision":        prop("string", "Repository revision or branch."),
				"path":            prop("string", "Repository file path."),
				"destination":     prop("string", "Workspace-relative download destination."),
				"dataset":         prop("string", "Dataset repository ID."),
				"config":          prop("string", "Dataset Viewer configuration."),
				"split":           prop("string", "Dataset split."),
				"offset":          prop("integer", "Dataset row offset."),
				"length":          prop("integer", "Dataset row count, bounded by huggingface.max_dataset_rows."),
				"where":           prop("string", "Dataset Viewer filter expression."),
				"paper_id":        prop("string", "Hugging Face paper ID."),
				"job_id":          prop("string", "Hugging Face Job ID."),
				"tail":            prop("integer", "Maximum number of log lines for a non-blocking job log snapshot."),
				"hardware":        prop("string", "Job hardware tier; must be in huggingface.allowed_hardware."),
				"timeout_minutes": prop("integer", "Job timeout, bounded by the configured maximum."),
				"script":          prop("string", "Python or UV script content for a Job."),
				"image":           prop("string", "Container image for a container Job."),
				"command":         map[string]interface{}{"type": "array", "items": map[string]interface{}{"type": "string"}, "description": "Executable and arguments for a container Job."},
				"arguments":       map[string]interface{}{"type": "array", "items": map[string]interface{}{"type": "string"}, "description": "Arguments passed to the Job command or script."},
				"title":           prop("string", "Discussion title."),
				"body":            prop("string", "Discussion or comment body."),
				"number":          prop("integer", "Discussion number."),
				"private":         prop("boolean", "Create a private repository when writes are enabled."),
				"local_path":      prop("string", "Local workspace path to upload."),
				"message":         prop("string", "Upload commit message."),
	...
	if ff.GrafanaEnabled {
		tools = append(tools, tool("grafana",
			...
			schema(map[string]interface{}{
				...
				"from":            prop("string", "Query time range start for query, e.g. now-1h or an epoch timestamp in milliseconds; defaults to now-1h"),
				"to":              prop("string", "Query time range end for query, e.g. now or an epoch timestamp in milliseconds; defaults to now"),
				"format":          prop("string", "Optional query result format such as time_series or table"),
				"max_data_points": map[string]interface{}{"type": "integer", "description": "Optional maxDataPoints value for query rendering resolution"},
				"interval_ms":     map[string]interface{}{"type": "integer", "description": "Optional intervalMs value for query rendering resolution"},
		...
		tools = append(tools, tool("remote_control",
			...
			schema(map[string]interface{}{
				"operation": map[string]interface{}{
					...
					"description": "Operation to perform",
					"enum": []string{
						"list_devices", "device_status", "execute_command", "shell_session_start", "shell_session_read", "shell_session_input", "shell_session_stop", "shell_session_list", "read_file", "write_file", "file_patch", "list_files", "sysinfo", "revoke_device", "edit_file", "json_edit", "yaml_edit", "xml_edit", "file_search", "file_read_advanced",
						"desktop_screenshot", "desktop_permission_request", "desktop_input",
						"desktop_list_displays", "desktop_list_windows", "desktop_active_window", "desktop_host_info",
						"desktop_ui_tree", "desktop_ui_action",
						"desktop_browser_connect", "desktop_browser_snapshot", "desktop_browser_action", "desktop_browser_disconnect",
				...
				"command":         prop("string", "Shell command to execute (for execute_command or shell_session_start)"),
				"cwd_id":          prop("string", "AgoDesk working-directory root id for shell_session_start."),
				"initial_wait_ms": prop("integer", "Initial read wait after shell_session_start. This is not the session lifetime."),
				"session_id":      prop("string", "Shell session id for shell_session_read/input/stop."),
				"offset":          prop("integer", "Output offset for shell_session_read."),
				"limit":           prop("integer", "Maximum records, bytes, or characters for list/read operations."),
				"wait_ms":         prop("integer", "Long-poll wait for shell_session_read."),
				"input":           prop("string", "Input to send to shell_session_input."),
				"path":            prop("string", "File or directory path on the remote device"),
				"root_id":         prop("string", "Agodesk file-access root id for read_file, write_file, file_patch, list_files, or file_search. When set, path is relative to that root."),
				"content":         prop("string", "File content to write (for write_file) or text to insert (for edit_file insert_after/insert_before/append/prepend)"),
				"expected_sha256": prop("string", "Required current SHA-256 of the target file for file_patch."),
				"dry_run":         prop("boolean", "Dry-run file_patch first. Defaults to true."),
				"patches": map[string]interface{}{
				...
				"absolute":     prop("boolean", "Set true for absolute mouse_move coordinates."),
				"button":       prop("string", "Mouse button for desktop_input mouse_click: left, right, or middle."),
				"input_action": prop("string", "Mouse click action for desktop_input mouse_click, e.g. click, down, or up. Preferred over the generic action field."),
		...
		tools = append(tools, tool("virtual_computers",
			...
			schema(map[string]interface{}{
				"operation": map[string]interface{}{
					...
					"enum":        []string{"status", "list_machines", "get_machine", "launch", "destroy", "exec", "extend", "fork", "screenshot", "upload", "download", "list_templates", "publish", "list_volumes", "get_volume", "create_volume", "delete_volume", "save_machine", "run_shell_task", "run_desktop_task", "list_agent_tasks", "get_agent_task", "cancel_agent_task"},
				},
				"machine_id":      prop("string", "Machine ID for machine-scoped operations."),
				"id":              prop("string", "Compatibility alias for the operation-specific identifier."),
				"template":        prop("string", "Template for launch, e.g. python or desktop."),
				"name":            prop("string", "Required template name for publish."),
				...
				"filename":        prop("string", "Safe filename for upload; boringd stores it below /root."),
				"path":            prop("string", "Remote file path for upload/download."),
				"remote_path":     prop("string", "Compatibility alias for the download path."),
	...
	if ff.VirtualDesktopEnabled {
		tools = append(tools, tool("virtual_desktop",
			"Control AuraGo's first-party browser virtual desktop. Use this to create or update desktop files, install generated JavaScript apps, pin widgets, open apps, and notify the user inside the desktop. "+
				...
				"Generated apps and widgets must bundle assets locally inside the desktop workspace; do not rely on external CDNs. Widgets remain stricter and should not fetch arbitrary third-party APIs directly. "+
			...
			schema(map[string]interface{}{
				...
				"widget": map[string]interface{}{
					"type":                 "object",
					"description":          "Widget payload for upsert_widget: id, app_id, type, title, icon, entry, runtime, permissions, x, y, w, h, config. icon follows the same icon_catalog rules as app manifests and is inferred from id/title/type/entry/app_id when omitted.",
					"additionalProperties": true,
				},
			}, "operation"),
		))
