		...
		tools = append(tools, tool("yepapi_youtube",
			...
			schema(map[string]interface{}{
				"operation": map[string]interface{}{
					...
					"enum":        []string{"search", "video", "video_info", "metadata", "transcript", "subtitles", "comments", "channel", "channel_videos", "channel_shorts", "channel_livestreams", "channel_live", "channel_playlists", "channel_community", "channel_about", "channel_search", "channel_channels", "channel_store", "playlist", "playlist_info", "trending", "related", "screenshot", "shorts", "shorts_info", "suggest", "hashtag", "post", "post_comments", "home", "hype", "resolve"},
				},
				"query":       prop("string", "Search query (for search, suggest, shorts, and channel_search operations)"),
				"video_id":    prop("string", "YouTube video ID (for video, video_info, metadata, transcript, subtitles, comments, related, screenshot, shorts_info operations)"),
				"channel_id":  prop("string", "YouTube channel ID (for channel and channel_* operations)"),
				"playlist_id": prop("string", "YouTube playlist ID (for playlist and playlist_info operations)"),
				"url":         prop("string", "YouTube URL (for resolve operation)"),
				"tag":         prop("string", "Hashtag/tag without # (for hashtag operation)"),
				"post_id":     prop("string", "YouTube community post ID (for post and post_comments operations)"),
				"country":     prop("string", "Optional country code for feed-style operations"),
				"language":    prop("string", "Optional language code for feed-style operations"),
		...
		tools = append(tools, tool("yepapi_tiktok",
			...
			schema(map[string]interface{}{
				"operation": map[string]interface{}{
					...
					"enum":        []string{"search", "search_user", "search_challenge", "search_photo", "video", "user", "user_posts", "user_followers", "user_following", "user_favorites", "user_reposts", "user_story", "comments", "comment_replies", "music", "music_videos", "challenge", "challenge_videos"},
				},
				"query":      prop("string", "Search query (for search, search_user, search_challenge, search_photo operations)"),
				"url":        prop("string", "TikTok video or music URL (for video, comments, music, music_videos operations)"),
				"username":   prop("string", "TikTok username/unique_id (for user and user_* operations)"),
				"name":       prop("string", "Challenge name (for challenge and challenge_videos operations)"),
				"comment_id": prop("string", "TikTok comment ID (for comment_replies operation)"),
				"limit":      map[string]interface{}{"type": "integer", "description": "Max results to return (default: 10)"},
			}, "operation"),
		))
	}
	...
	if ff.YepAPIInstagramEnabled {
		tools = append(tools, tool("yepapi_instagram",
			...
			schema(map[string]interface{}{
				...
				"query":           prop("string", "Search query for search operation (alias for 'search_query')"),
				"search_query":    prop("string", "Search query for search operation"),
				"username":        prop("string", "Alias for 'username_or_url' for user and user_* operations"),
				"username_or_url": prop("string", "Instagram username or profile URL for user and user_* operations. Supported by the API directly."),
		...
		tools = append(tools, tool("yepapi_amazon",
			...
			schema(map[string]interface{}{
				"operation": map[string]interface{}{
					...
					"enum":        []string{"search", "product", "reviews", "product_offers", "products_by_category", "categories", "deals", "best_sellers", "influencer", "seller", "seller_reviews"},
				},
				"query":     prop("string", "Search query (for search operation)"),
				"asin":      prop("string", "Amazon ASIN product ID (for product, reviews, product_offers operations)"),
				"country":   map[string]interface{}{"type": "string", "description": "Amazon marketplace country code, e.g. 'US', 'UK', 'DE' (default: 'US')"},
				"category":  prop("string", "Category slug or browse node ID (for products_by_category, deals, best_sellers operations)"),
				"handle":    prop("string", "Amazon influencer handle (for influencer operation)"),
				"seller_id": prop("string", "Amazon seller ID (for seller and seller_reviews operations)"),
				"limit":     map[string]interface{}{"type": "integer", "description": "Max results to return where supported (default: endpoint-specific)"},
				"page":      map[string]interface{}{"type": "integer", "description": "Page number for paginated operations"},
