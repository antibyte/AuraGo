...
func buildCoreToolSchemas(ff ToolFeatureFlags, execSkillProps map[string]interface{}) []openai.Tool {
	tools := []openai.Tool{
		...
		tool("analyze_image",
			...
			schema(map[string]interface{}{
				"file_path": prop("string", "Path to a local image file (JPEG, PNG, GIF, WebP). Not supported by Agnes AI."),
				"image_url": prop("string", "Publicly reachable HTTP(S) image URL. Required when the Vision provider is Agnes AI."),
