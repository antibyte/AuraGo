...
func (s *Store) migrate() error {
	...
	if version > 2 {
		return fmt.Errorf("SIP call store version %d is newer than supported version 2", version)
	}
	tx, err := s.db.Begin()
	if err != nil {
...
(id,direction,remote_party,started_at,answered_at,ended_at,state,end_reason,backend,session_id,persist_transcripts)
VALUES(?,?,?,?,?,?,?,?,?,?,?)
ON CONFLICT(id) DO UPDATE SET
 ...
 persist_transcripts=excluded.persist_transcripts`,
		...
		nullableMillis(call.EndedAt), call.State, call.EndReason, call.Backend, call.SessionID, call.persistTranscripts)
	if err != nil {
...
func (s *Store) List(ctx context.Context, limit int) ([]CallRecord, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT id,direction,remote_party,started_at,answered_at,ended_at,state,end_reason,backend,session_id,persist_transcripts
FROM sip_calls ORDER BY started_at DESC LIMIT ?`, limit)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&call.ID, &call.Direction, &call.RemoteParty, &started, &answered, &ended, &call.State, &call.EndReason, &call.Backend, &call.SessionID, &call.persistTranscripts); err != nil {
