...
- Vulkan is an additional Linux/amd64 backend, implemented by pinned
  ...
  Probe and worker enforce `GGML_VK_DISABLE_F16=1`: FP16 math corrupts Qwen LM
  text/audio-code generation on tested Renoir hardware. Keep GPU execution and
  GGUF weight quantization; `compute_precision=fp32` participates in the profile
  fingerprint and status. Image/default changes require requalification.
...
- MP3 decoding, duration and volume only establish technical output. Verify LM
  text coherence and listen to instrumental/vocal samples before quality acceptance.
