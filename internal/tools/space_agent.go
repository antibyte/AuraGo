...
const (
	...
	spaceAgentImageBuildRevision   = "20260502-aurago-python-instructions-api"
...
class AuragoInstructions(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        try:
            ...
            try:
                context.log.log(type="user", heading="", content=message, id=msg_id)
            except Exception:
                pass
            task = context.communicate(UserMessage(message=message, id=msg_id))
