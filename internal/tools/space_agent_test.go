...
func TestSpaceAgentBootstrapScriptCreatesManagedAdminUser(t *testing.T) {
	...
	for _, want := range []string{
		...
		"writeFile(path.join(rootPath, \"mod\", \"aurago\", \"inbox_poller\", \"ext\", \"js\", \"_core\", \"framework\", \"initializer.js\", \"initialize\", \"end\", \"aurago-inbox-poller.js\")",
...
func TestEnsureSpaceAgentHomeSeedsExpectedWorkspaceFiles(t *testing.T) {
	...
	for path, want := range map[string]string{
		...
		filepath.Join(home, "mod", "aurago", "inbox_poller", "ext", "js", "_core", "framework", "initializer.js", "initialize", "end", "aurago-inbox-poller.js"): "contains:submitPrompt",
...
func TestEnsureSpaceAgentCustomwareUserHomeSeedsL2WorkspaceFiles(t *testing.T) {
	...
	for path, want := range map[string]string{
		...
		filepath.Join(userHome, "mod", "aurago", "inbox_poller", "ext", "js", "_core", "framework", "initializer.js", "initialize", "end", "aurago-inbox-poller.js"): "contains:submitPrompt",
