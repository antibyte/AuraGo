...
func (s *SQLiteMemory) decodeJournalStringSlice(raw, field string) []string {
	var out []string
	...
	if err := json.Unmarshal([]byte(raw), &out); err != nil {
		if s != nil && s.logger != nil {
			s.logger.Warn("Journal JSON decode failed", "field", field, "error", err)
		}
	}
	return out
}

func (s *SQLiteMemory) decodeJournalStringIntMap(raw, field string) map[string]int {
...
func (s *SQLiteMemory) ConsolidateDuplicateAutoJournalErrors(opts JournalConsolidationOptions) (JournalConsolidationReport, error) {
	...
	for _, group := range groups {
		...
		var tags []string
		tags = s.decodeJournalStringSlice(group.tagsJSON, "journal_entries.tags")
...
func (s *SQLiteMemory) GetJournalEntries(from, to string, types []string, limit int) ([]JournalEntry, error) {
	...
	for rows.Next() {
		...
		e.Tags = s.decodeJournalStringSlice(tagsJSON, "journal_entries.tags")
...
func (s *SQLiteMemory) GetDailySummary(date string) (*DailySummary, error) {
	...
	d.KeyTopics = s.decodeJournalStringSlice(topicsJSON, "daily_summaries.key_topics")
	d.ToolUsage = s.decodeJournalStringIntMap(usageJSON, "daily_summaries.tool_usage")
	return &d, nil
}
