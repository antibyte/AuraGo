...
// API tests explicitly install the small Game Maker fixtures. This fallback is
// compiled only into tests; released binaries never read repository sources.
func TestMain(m *testing.M) {
	root, err := os.MkdirTemp("", "aurago-server-assets-")
	if err != nil {
		panic(err)
	}
	...
	if strings.HasPrefix(filepath.Base(root), "aurago-server-assets-") {
		os.RemoveAll(root)
	}
	os.Exit(code)
}
