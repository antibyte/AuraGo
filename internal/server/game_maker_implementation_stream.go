...
// Bound it explicitly, and retry only one incomplete stream or local deadline.
// Each attempt owns its stream; partial code never enters a file or the retry.
func (r *gameMakerAgentRunner) gameStarterCompletion(ctx context.Context, cfg *config.Config, client llm.ChatClient, run gamemaker.JobRun, system, prompt string) (agent.MinimalLoopResult, []openai.ChatCompletionMessage, error) {
	...
	for attempt := 0; ; attempt++ {
		...
		if err == nil || attempt > 0 || ctx.Err() != nil || (!timedOut && !errors.Is(err, agent.ErrIncompleteTextStream)) {
			return response, completion, err
		}
		if r.server.Logger != nil {
			r.server.Logger.Warn("game maker source stream interrupted; retrying once", "job_id", run.Job.ID, "timeout", timeout, "deadline_exceeded", timedOut, "error", err)
		}
