...
func agodeskProviderReferences(cfg *config.Config, providerID string) []agodesk.ProviderReferencePayload {
	...
	refs := []agodesk.ProviderReferencePayload{}
	add := func(path, role, value string) {
		if strings.TrimSpace(value) == id {
			refs = append(refs, agodesk.ProviderReferencePayload{Path: path, Role: role})
		}
	}
...
func upsertAgodeskProvider(s *Server, payload agodesk.ConfigProviderUpsertPayload) (agodesk.ConfigProviderEntryPayload, error) {
	...
	if providerID == "" {
		return agodesk.ConfigProviderEntryPayload{}, fmt.Errorf("provider id is required")
	}
