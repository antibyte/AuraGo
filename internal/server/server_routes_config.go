...
func (s *Server) registerConfigAPIRoutes(mux *http.ServeMux, sse *SSEBroadcaster) {
	...
	mux.HandleFunc("/api/mcp-secrets", handleMCPSecrets(s))
	...
	mux.HandleFunc("/api/composio/status", handleComposioStatus(s))
	mux.HandleFunc("/api/composio/test", handleComposioTest(s))
	mux.HandleFunc("/api/composio/toolkits", handleComposioToolkits(s))
	mux.HandleFunc("/api/composio/tools", handleComposioTools(s))
	...
	mux.HandleFunc("/api/manus/status", handleManusStatus(s))
	mux.HandleFunc("/api/manus/test", handleManusTest(s))
	mux.HandleFunc("/api/manus/projects", handleManusProjects(s))
	mux.HandleFunc("/api/manus/connectors", handleManusConnectors(s))
	mux.HandleFunc("/api/manus/skills", handleManusSkills(s))
	mux.HandleFunc("/api/huggingface/status", handleHuggingFaceStatus(s))
	mux.HandleFunc("/api/huggingface/test", handleHuggingFaceTest(s))
	mux.HandleFunc("/api/huggingface/jobs", handleHuggingFaceJobs(s))
	...
	// Keep the read-only connection probe separate from the legacy Telegram
	// test-message route, which may be preserved by local updater commits.
	mux.Handle("/api/telegram/test-connection", requireAdmin(s, handleTelegramConnectionTest(s)))
	...
	mux.HandleFunc("/api/homepage/sites", handleHomepageSites(s))
	mux.HandleFunc("/api/homepage/sites/", handleHomepageSiteByID(s))
	...
	registerKoofrHandlers(mux, s)
	registerWebDAVHandlers(mux, s)
	registerTelnyxHandlers(mux, s)
	registerTailscaleHandlers(mux, s)
	registerAIGatewayHandlers(mux, s)
	...
	mux.HandleFunc("/api/omniroute/status", handleOmniRouteStatus(s))
	mux.HandleFunc("/api/omniroute/test", handleOmniRouteTest(s))
	mux.HandleFunc("/api/omniroute/start", handleOmniRouteStart(s))
	mux.HandleFunc("/api/omniroute/stop", handleOmniRouteStop(s))
	...
	mux.HandleFunc("/api/dashboard/memory/curation", handleDashboardMemoryCuration(s))
	...
	mux.Handle("/api/operational-issues", requireAdmin(s, handleOperationalIssues(s)))
	mux.Handle("/api/operational-issues/", requireAdmin(s, handleOperationalIssues(s)))
	mux.Handle("/api/cron", requireAdmin(s, handleCronAPI(s)))
	mux.Handle("/api/cron/", requireAdmin(s, handleCronAPI(s)))
	...
	mux.HandleFunc("/api/knowledge-graph/edge/claims", handleKnowledgeGraphEdgeClaims(s))
	mux.HandleFunc("/api/knowledge-graph/conflicts", handleKnowledgeGraphConflicts(s))
	mux.Handle("/api/knowledge-graph/conflicts/resolve", requireAdmin(s, handleKnowledgeGraphConflictResolve(s)))
