...
func TestIsAdminProtectedPathIncludesLegacyCronAPI(t *testing.T) {
	...
	for _, path := range []string{"/api/cron", "/api/cron/job-1", "/api/dashboard/memory/reflection/run"} {
...
func TestAuthMiddlewareRejectsPrinterCameraStreamWithGenericDesktopEmbedTokenQuery(t *testing.T) {
	t.Parallel()
	...
	if rec.Code != http.StatusUnauthorized {
		t.Fatalf("camera stream with generic desktop token status = %d, want 401; body=%s", rec.Code, rec.Body.String())
	}
}

func TestAuthMiddlewareAllowsPrinterCameraStreamWithResourceScopedDesktopEmbedTokenQuery(t *testing.T) {
	t.Parallel()
	...
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusNoContent {
		t.Fatalf("camera stream with resource token status = %d, want 204; body=%s", rec.Code, rec.Body.String())
	}
}
...
func TestAuthMiddlewareRejectsPrinterCameraStreamWithDesktopEmbedTokenReferer(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/3d-printers/printer-1/camera/stream", nil)
...
func TestAuthMiddlewareRejectsDesktopEmbedTokenForOtherAPIs(t *testing.T) {
	t.Parallel()
	...
	req := httptest.NewRequest(http.MethodGet, "/api/config?desktop_token="+url.QueryEscape(token), nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusUnauthorized {
		t.Fatalf("non-camera API with desktop embed token status = %d, want 401", rec.Code)
	}
}
