...
func TestLooperRunHandlerDetachesExecutionContextFromHTTPRequest(t *testing.T) {
	...
	source := string(sourceBytes)
	if !strings.Contains(source, "context.WithTimeout(context.Background(), looperRunTimeout(req.MaxIter))") {
		t.Fatal("looper run context must outlive the short /run HTTP request")
	}
	if strings.Contains(source, "context.WithTimeout(r.Context(), looperRunTimeout(req.MaxIter))") {
