...
func ToolPermissionMatrix() []ToolPermission {
	return []ToolPermission{
		...
		{Name: "cyd_display", ConfigGate: "cyd.enabled", Capabilities: []Capability{CapabilityRead, CapabilityChange, CapabilityNetwork}},
		{Name: "send_notification", ConfigGate: "notifications.* or cyd.enabled", Capabilities: []Capability{CapabilityNetwork}},
		{Name: "composio_call", ConfigGate: "composio.enabled + composio_api_key", ReadOnlyGate: "composio.read_only", Capabilities: []Capability{CapabilityRead, CapabilityWrite, CapabilityChange, CapabilityDelete, CapabilityNetwork}},
		...
		{Name: "manus", ConfigGate: "manus.enabled + manus_api_key", ReadOnlyGate: "manus.read_only", Capabilities: []Capability{CapabilityRead, CapabilityWrite, CapabilityChange, CapabilityNetwork, CapabilityHost}},
...
func HomeLabIntegrationMatrix() []HomeLabIntegration {
	return []HomeLabIntegration{
		...
		{Name: "cyd", ConfigGate: "cyd.enabled", CredentialSource: "token-manager scope cyd", AllowsLocalNetwork: true, HasTestEndpoint: true},
...
func RouteContractManifest() []RouteContract {
	return []RouteContract{
		...
		{Pattern: "/api/dashboard/audit", Methods: []string{"GET", "DELETE"}, Auth: "session-admin-delete", Category: "dashboard", ContentTypes: []string{"application/json"}},
		...
		{Pattern: "/api/cyd/snapshot", Methods: []string{"GET"}, Auth: "bearer-scope-cyd", Category: "cyd"},
		...
		{Pattern: "/api/cyd/firmware/provision", Methods: []string{"POST"}, Auth: "session", Category: "cyd", ContentTypes: []string{"application/json"}},
		{Pattern: "/api/cyd/firmware/", Methods: []string{"GET"}, Auth: "session", Category: "cyd"},
...
func NetworkClientInventory() []NetworkClientUse {
	return []NetworkClientUse{
		{Path: "internal/acestep/", Classification: "digest-pinned-managed-docker-runtime-and-fixed-private-api", AllowsLocalNet: true, Credentialed: true},
		...
		{Path: "internal/onvif/", Classification: "bounded-private-onvif-discovery-and-setup", AllowsLocalNet: true, Credentialed: true},
...
func DBMigrationManifest() []DBMigrationDomain {
	return []DBMigrationDomain{
		...
		{Domain: "manus", PackagePath: "internal/manus", SchemaVersioned: true, OwnsRuntimeData: true},
...
func SecurityBoundaryManifest() []SecurityBoundary {
	return []SecurityBoundary{
		...
		{
			Name:         "local-llm-runtime-key-volume",
			Boundary:     "Vault local_llm_runtime_api_key -> managed llama.cpp sidecar",
			Enforcement:  "Docker archive seeds a mode-0600 UID/GID-65532 file into an ephemeral protected volume mounted read-only; no environment, argument, log, YAML, or API exposure",
			TestCoverage: "internal/localllm/localllm_test.go and internal/agent/local_llm_docker_protection_test.go",
		},
		{
