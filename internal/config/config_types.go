...
// ManifestRoutingConfig holds optional routing hints sent to the Manifest gateway.
type ManifestRoutingConfig struct {
	Enabled         bool              `yaml:"enabled" json:"enabled"`                   // enable AuraGo-to-Manifest routing headers
	SpecificityMode string            `yaml:"specificity_mode" json:"specificity_mode"` // "off" (default), "fixed", or "auto"
	Specificity     string            `yaml:"specificity" json:"specificity"`           // fixed Manifest specificity category
	Headers         map[string]string `yaml:"headers" json:"headers"`                   // additional safe headers for Manifest header tiers
}
