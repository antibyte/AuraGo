...
func PhaseGuidance(stage, dimension string) string {
	...
	if dimension == "2d" {
		common += ` Phaser 4: subclass GameScene with setup(), step(dt), action(), tick(), paintHUD(); never override update/create. Assign this.player; preserve jump in action and call super.step(dt) when extending movement. Spawn the whole collider above ground. A stomp proves target damage/removal, not game outcome. body(x,y,w,h,color,fixed,role) automatically fits/follows/animates planned art. Never add another sprite to it; manual art requires an empty role and no super.setup creating another player. Roles: player/enemy/projectile/item/obstacle/goal. Use real GameObjects in collider/overlap and fixed=false for movers. Neither Arcade body type has setPosition: use object.setPosition plus body.reset(x,y), or updateFromGameObject for static proxies. Register persistent groups/overlaps once; never load a sheet as one image. New helpers: configureLevels([{id,title},...]) and levelIndex select distinct layouts in setup; configureWorld(width,height) after creating player enables camera follow. setCheckpoint(x,y), damagePlayer() consume a life on real harmful contact with protected respawn; builder health rules remain their own authority. feedback(name,object,material) supplies contact feedback; end(won) shows result/Continue, restartGame() resets campaign. Scene JSON levels also work. Use scope=full.`
	} else {
