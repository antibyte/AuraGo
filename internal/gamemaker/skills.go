...
func PhaseGuidance(stage, dimension string) string {
	...
	if dimension == "2d" {
		common += ` Phaser 4: subclass GameScene using setup(), step(deltaSeconds), action(), tick(), paintHUD(); never override update/create. Assign this.player. Keep movement in step(deltaSeconds) (call super.step(deltaSeconds) when extending it) and preserve the template jump in action(); an inherited update does not restore an overridden step. Spawn the whole player collider above the ground, not intersecting it. A stomp test measures a contacted enemy being removed/hurt, not the whole game outcome; use outcome only for win/loss tests. body(x,y,w,h,color,fixed,role) creates, fits, follows and animates planned artwork automatically. Do not add another createAsset/createAssembly for the same body: hiding its collision rectangle does not remove its sprite. Manual artwork requires an explicitly empty role and no super.setup() that creates a planned player. Roles include player, enemy, projectile, item, obstacle and goal. Pass actual physics GameObjects to collider/overlap, not wrapper records. Use fixed=false for moving bodies. Neither Arcade body type has setPosition; use object.setPosition and body.reset(x,y), or updateFromGameObject for static proxies. Use persistent groups for spawned objects and register overlap once. Never load a library sheet as a single image. Use scope=full.`
	} else {
		...
		common += ` When changing shooting, keep fire() and observeTargets() on the same aimRay(caster) path; preserve read-only aim_ray origin/direction (observer x/y=ground plane, z=altitude). The target test follows that real ray using normal inputs; never manufacture hits, target removal or test verdicts.`
	}
	if stage == "repair" {
