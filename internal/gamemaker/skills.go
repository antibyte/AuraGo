...
func PhaseGuidance(stage, dimension string) string {
	...
	if stage == "repair" {
		common += ` Repair the diagnostics, validate, then stop. For no_target, bind its exact ID/role to the real 2D object with body(..., role) or __gmRole; never fake metrics.`
	}
	return common
}
