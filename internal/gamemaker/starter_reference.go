...
Hooks: setup(api) after assets load, step(dtSeconds,api) adds rules after normal movement/simulation, action(api) returns false to override primary input, reset(api), hud(api) returns HUD text, dispose(api). Store custom state in your closure and reset it in reset; use setup for resources needing loaded assets. Do not add another frame/input loop.
