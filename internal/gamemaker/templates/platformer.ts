...
class Platformer extends GameScene {
  setup() {
    this.player = this.body(160, 450, 28, 40, 0x5eead4);
    const ground = this.body(480, 520, 960, 40, 0x334155, true);
    this.physics.add.collider(this.player, ground);
    for (const [x,y] of [[350,420],[580,340],[760,260]]) this.physics.add.collider(this.player, this.body(x,y,150,20,0x475569,true));
    const coin = this.body(280,470,20,20,0xfacc15,true);
    this.physics.add.overlap(this.player,coin,()=>{coin.destroy();this.state.hits++;this.state.score++;});
    const danger = this.body(870,485,40,30,0xfb7185,true);
    this.physics.add.overlap(this.player,danger,()=>this.end());
    const goal = this.body(820,210,30,60,0xa78bfa,true);
