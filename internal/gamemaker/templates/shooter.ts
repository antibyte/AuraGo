...
class Shooter extends GameScene {
  ...
  setup() { if (this.setupScene()) return;
    this.configureLevels([{id:'approach',title:'First wave'},{id:'crossfire',title:'Crossfire'}]);this.state.lives=3;
    ...
    this.physics.add.overlap(this.shots, this.enemies, (shot: any, enemy: any) => {
      ...
      if(this.state.hits>=(this.levelIndex===0?6:9))this.end(true);
    });
  ...
  spawn() {
    ...
    const enemy = this.body(x, 100, 32, 32, 0xfb7185, false, "enemy");
