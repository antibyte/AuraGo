	...
	return []LooperPreset{
		{
			...
			Finish:        "Create or update a Writer document with the final story, then open it in the Writer app on the Desktop with virtual_desktop open_in_app so the user sees the result.",
		...
		},
		{
			...
			Finish:        "Create or update a Writer document with the final polished document, then open it in the Writer app on the Desktop with virtual_desktop open_in_app.",
			FinishContext: "last_action_test",
			MaxIter:       8,
		},
		{
		...
		},
		{
			...
			ExitCond: `Evaluate whether the current result meets the exit criteria. Reply with valid JSON only (no extra text):
{"decision": true/false, "reason": "short explanation why you decided this way", "confidence": 0.0-1.0}`,
