...
func buildCodeStudioContainerWorkspaceRepairScript() string {
	return strings.Join([]string{
		...
		"  if chgrp -R developer /workspace/Games 2>/dev/null; then",
		"    find /workspace/Games -type d -exec chmod 0775 {} + 2>/dev/null || true",
		"    find /workspace/Games -type f -exec chmod 0664 {} + 2>/dev/null || true",
		"  else",
		"    find /workspace/Games -type d -exec chmod a+rwx {} + 2>/dev/null || true",
		"    find /workspace/Games -type f -exec chmod a+rw {} + 2>/dev/null || true",
		"  fi",
		"fi",
		"if [ ! -w /workspace ]; then",
