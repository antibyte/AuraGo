...
func TestDesktopSysmonBytesI18n(t *testing.T) {
	t.Parallel()

	source := readDesktopAssetText(t, "js/desktop/core/widget-sysmon-runtime.js")
	for _, want := range []string{
		...
		if !strings.Contains(source, want) {
			t.Fatalf("sysmon bytes i18n missing marker %q", want)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(source, forbidden) {
			t.Fatalf("sysmon still hardcodes %q", forbidden)
		}
	}
