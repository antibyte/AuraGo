...
func TestDesktopHTMLLoadsFragmentedAppsOnlyThroughMainLoader(t *testing.T) {
	...
	if !strings.Contains(html, `<script defer src="/js/desktop/main.js?v={{.BuildVersion}}-desktop-20260516e"></script>`) {
...
func TestDesktopMainLoaderBumpsCacheAfterStoreIframeChanges(t *testing.T) {
	...
	main := rawDesktopAssetText(t, "js/desktop/main.js")
	if !strings.Contains(main, "var assetV = v + '-desktop-20260521-store-iframe-dashboard';") {
