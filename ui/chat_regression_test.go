...
func TestChatFrontend_IntegrationsDrawerRemainsWired(t *testing.T) {
	...
	for _, forbidden := range []string{"new URL(window.location.href)", "url.port =", "url.protocol ="} {
		if strings.Contains(drawerJS, forbidden) {
...
func TestChatFrontend_MobileHeaderControlsRemainTappable(t *testing.T) {
	...
	for _, marker := range []string{
		...
		"control.addEventListener('touchmove'",
...
func TestChatFrontend_HeaderControlsRemainNormalizedAcrossThemes(t *testing.T) {
	...
	for _, marker := range []string{
		...
		".app-header .header-actions .btn-warnings",
		"overflow: visible;",
		".app-header .header-actions .warnings-badge",
		"top: 1px;",
		"right: 1px;",
...
func TestChatEightBitPixelationRestoresOriginalImages(t *testing.T) {
	...
	for _, marker := range []string{
		`[data-theme="8bit"] #chat-box::after`,
		`image-rendering: pixelated`,
		`data-aurago8bit-bg="pixelated-4x4"`,
	} {
...
func TestChatUIEmojiIconsAreImageAssets(t *testing.T) {
	...
	if got := strings.Count(iconsJS, "shape: "); got != 103 {
		t.Fatalf("%s has %d explicit icon shapes, want 103", iconsPath, got)
	}
	...
	requiredIconKeys := []string{
		...
		"theme-dark-sun", "theme-ocean", "theme-sandstorm", "theme-papyrus", "theme-threedee", "theme-galaxy", "theme-black-matrix", "theme-8bit",
	...
	if len(iconFiles) != 103 {
		t.Fatalf("%s has %d generated PNG icons, want 103", iconDir, len(iconFiles))
	}
