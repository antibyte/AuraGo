...
- Live Speech's shared panel owns one `AuraRealtimeSpeechAvatar` per mount.
  ...
  While resolving a persona or loading its animation, leave the portrait empty.
  Do not show the static PNG as a stand-in. Show only the selected persona.
  Use the selected PNG only for reduced motion or an actual load failure, and
  the Custom image only for custom personas or an unknown key.
...
- MeshCore Config uses the shared saved/draft path, explicit device/channel binding confirmation, full-key trust lists and separate reply/proactive permissions. Its inbox and discovered device names render external text with `textContent`; pairing PINs are transient. Discovery shows a live spinner and selectable results; selecting a device updates only the draft Bluetooth address/transport, never trust or pairing automatically. Device tests and pairing use saved settings, bounded fetches and inline status. Removing a device channel requires a confirmation, a clean Config draft and the active identity/slot/binding conversation; use the Messenger manager API, then rebase the saved MeshCore draft from device status. Removing an orphaned channel rule remains a draft-only Save action. Keep hardware acceptance visibly unverified and all locale files under `lang/config/meshcore/` complete.
