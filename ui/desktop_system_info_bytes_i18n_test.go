...
func TestDesktopSystemInfoBytesI18n(t *testing.T) {
	t.Parallel()
	...
	for _, want := range []string{
		...
		"'desktop.gib'",
		"'desktop.tib'",
		"formatBytes(memory.used, instance.context)",
		"formatBytes(network.bytes_sent, instance.context)",
	} {
		if !strings.Contains(source, want) {
			t.Fatalf("system-info bytes i18n missing marker %q", want)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(source, forbidden) {
			t.Fatalf("system-info still hardcodes %q", forbidden)
		}
	}
