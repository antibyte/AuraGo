(function () {
    ...
    function render(container, windowId, context = {}) {
        ...
        inst.cleanup.push(NS().data.subscribe(snapshot=>{
            ...
            inst.hud.update(snapshot,rows);inst.city?.setData(rows);
        ...
        async function loadCity(){
            try{
                ...
                const city=await module.createCity(canvasHost,{
                    ...
                    assetURL:file=>versioned('/3d/system-world/v1/'+file),
                    ...
                    onReady:()=>{if(!inst.disposed)inst.hud.ready();},
                ...
                inst.city=city;city.setData(inst.entities);city.setMode(inst.mode);syncVisibility();
            }catch(_){
    ...
    function dispose(windowId) {
        ...
        inst.cleanup.forEach(fn=>fn());inst.city?.dispose();inst.hud.dispose();inst.root.remove();instances.delete(windowId);
    }
    ...
    function inspect(windowId) {
        ...
        return inst?{selected:inst.selected,visible:inst.visible,mode:inst.mode,entityCount:inst.entities.length,raf:!!inst.raf,...inst.city?.stats()}:null;
    }
