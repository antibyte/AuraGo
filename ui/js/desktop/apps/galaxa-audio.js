                    ...
                    for (const vn of ['bass', 'lead', 'harmony', 'arpeggio']) {
                        ...
                        for (const n of voice.notes) {
                            if (n.f > 0) {
                                ...
                                g.gain.setValueAtTime(voice.vol * (ctx.G.muted ? 0 : 1) * (vn === 'arpeggio' ? Math.min(1, Math.max(0, (this.intensity - 4) / 4)) : vn === 'lead' ? Math.min(1, Math.max(0.3, this.intensity / 8)) : vn === 'harmony' ? Math.min(1, Math.max(0, (this.intensity - 2) / 5)) : 1), a.currentTime + offset);
