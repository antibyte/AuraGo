(function () {
    ...
    GC.createAudioSfx = function (ctx) {
        ...
        const audio = () => ctx.audio();
            ...
            stage() { [523, 659, 784, 1047].forEach((f, i) => { ctx.scheduleSfx(() => beep('sine', f, f, 0.2, 0.25), i * 120); }); },
            challenge() { [440, 554, 659, 880, 1109, 1319].forEach((f, i) => { ctx.scheduleSfx(() => beep('square', f, f, 0.15, 0.2), i * 80); }); },
            extra() { beep('sine', 1200, 1200, 0.2, 0.3); },
            rescue() { beep('sine', 880, 880, 0.2, 0.25); ctx.scheduleSfx(() => beep('sine', 1100, 1100, 0.2, 0.25), 100); },
            beam() { beep('sawtooth', 200, 200, 0.5, 0.15); },
            perfect() { [523, 659, 784, 1047, 1319, 1568].forEach((f, i) => { ctx.scheduleSfx(() => beep('sine', f, f, 0.15, 0.3), i * 60); }); },
            puCollectRarity(rarity, panX) {
                ...
                notes.forEach((f, i) => ctx.scheduleSfx(() => beep('sine', f, f, 0.06, 0.18 + i * 0.02, panX), i * gap));
            ...
            bossWarning() { beep('sawtooth', 440, 220, 0.5, 0.3); ctx.scheduleSfx(() => beep('sawtooth', 440, 220, 0.5, 0.3), 500); },
            shieldHit() { const _p = pv(), _v = vv(); beep('triangle', 2000 * _p, 4000 * _p, 0.05, 0.3 * _v); beep('sine', 3000 * _p, 1500 * _p, 0.08, 0.2 * _v); },
            respawn() { if (ctx.G.muted) return; beep('sine', 200, 800, 0.3, 0.25); ctx.scheduleSfx(() => { if (ctx.G.muted) return; beep('sine', 600, 1200, 0.2, 0.2); }, 80); ctx.scheduleSfx(() => { if (ctx.G.muted) return; beep('sine', 1800, 3600, 0.28, 0.09); beep('triangle', 2400, 4800, 0.22, 0.06); noise(0.18, 0.05, 9000); }, 30); },
            shieldBreak() { noise(0.2, 0.5 * vv(), 3000); beep('sawtooth', 200 * pv(), 100, 0.15, 0.4 * vv()); },
            bossJingle() { [220, 262, 330, 220, 165, 220].forEach((f, i) => { ctx.scheduleSfx(() => beep('sawtooth', f, f, 0.15, 0.2 + i * 0.02), i * 100); }); },
            stageClear() { this.stageClearFanfare(); },
            puUpgrade(panX) { [800, 1000, 1200, 1400, 1600].forEach((f, i) => { ctx.scheduleSfx(() => beep('sine', f, f, 0.05, 0.25, panX), i * 30); }); },
            weaponUp() { [600, 800, 1000, 1200].forEach((f, i) => { ctx.scheduleSfx(() => beep('triangle', f, f, 0.08, 0.2), i * 60); }); },
            ...
            miniBossWarning() { beep('sawtooth', 330, 165, 0.4, 0.3); ctx.scheduleSfx(() => beep('sawtooth', 330, 165, 0.4, 0.3), 400); },
            bossHitSFX(panX) { const _p = pv(), _v = vv(); beep('sawtooth', 280 * _p, 60 * _p, 0.12, 0.45 * _v, panX); noise(0.1, 0.35 * _v, 900, panX); },
            warpJump() { beep('sawtooth', 180, 3600, 0.35, 0.45); beep('sine', 90, 3000, 0.28, 0.35); ctx.scheduleSfx(() => noise(0.15, 0.3, 4000), 250); },
            coinInsert() { beep('triangle', 440, 880, 0.06, 0.45); ctx.scheduleSfx(() => beep('triangle', 880, 1760, 0.06, 0.45), 70); },
            comboBreak() { beep('sawtooth', 440 * pv(), 200, 0.18, 0.2 * vv()); },
            killStreak() { [880, 1100, 1320, 1760].forEach((f, i) => { ctx.scheduleSfx(() => beep('sine', f, f, 0.09, 0.28), i * 55); }); },
            freeze(panX) { const _p = pv(), _v = vv(); beep('sine', 1200 * _p, 3600 * _p, 0.06, 0.35 * _v, panX); beep('triangle', 900 * _p, 2800 * _p, 0.05, 0.28 * _v, panX); ctx.scheduleSfx(() => { beep('triangle', 400, 180, 0.09, 0.15, panX); noise(0.12, 0.18, 7000, panX); }, 100); },
            ...
            comboMilestone(n, panX) { [880, 1100, 1320, 1760, 2200].forEach((f, i) => { ctx.scheduleSfx(() => beep('sine', f, f, 0.08, 0.2 + n * 0.03, panX), i * 40); }); },
            ...
            relicActivate() { beep('sine', 440, 440, 0.3, 0.25); beep('triangle', 660, 660, 0.25, 0.15); beep('sine', 880, 880, 0.2, 0.1); ctx.scheduleSfx(() => beep('sine', 1100, 1100, 0.15, 0.08), 150); },
            nearMiss(panX) { noise(0.05, 0.15, 7000, panX); beep('sine', 80, 60, 0.12, 0.2); },
            bossPhaseTrans() { beep('sawtooth', 100, 50, 0.6, 0.4); noise(0.4, 0.3, 800); ctx.scheduleSfx(() => beep('sawtooth', 200, 100, 0.3, 0.25), 200); },
            mutationStart() { beep('sawtooth', 180, 90, 0.4, 0.3); noise(0.3, 0.15, 3000); beep('sine', 60, 40, 0.5, 0.2); },
            comboExtend() { [600, 800, 1000, 1200].forEach((f, i) => { ctx.scheduleSfx(() => beep('triangle', f, f, 0.06, 0.2), i * 30); }); },
            ...
            parrySuccess(panX) { if (ctx.G.muted) return;
                ...
                ctx.scheduleSfx(() => beep('sine', 1400, 900, 0.08, 0.16 * _v, panX), 50);
            },
            ...
            superActivate(shipType, panX) {
                const _v = vv();
                if (shipType === 'classic') { beep('sawtooth', 200, 1400, 0.3, 0.4 * _v, panX); beep('square', 100, 300, 0.25, 0.2 * _v, panX); ctx.scheduleSfx(() => noise(0.4, 0.5 * _v, 800, panX), 250); }
                else if (shipType === 'interceptor') { beep('sine', 300, 3000, 0.3, 0.4 * _v, panX); beep('sawtooth', 150, 2000, 0.25, 0.25 * _v, panX); ctx.scheduleSfx(() => noise(0.15, 0.3 * _v, 5000, panX), 200); }
                else if (shipType === 'heavy') { beep('sawtooth', 80, 60, 0.5, 0.6 * _v, panX); noise(0.5, 0.5 * _v, 400, panX); beep('square', 120, 80, 0.3, 0.3 * _v, panX); }
                else if (shipType === 'stealth') { beep('sine', 600, 1800, 0.35, 0.35 * _v, panX); beep('triangle', 900, 2400, 0.3, 0.25 * _v, panX); ctx.scheduleSfx(() => { beep('sine', 1200, 1200, 0.2, 0.2 * _v, panX); beep('sine', 1500, 1500, 0.2, 0.15 * _v, panX); beep('sine', 1800, 1800, 0.2, 0.12 * _v, panX); }, 150); }
                else { beep('sawtooth', 150, 800, 0.4, 0.45 * _v, panX); noise(0.3, 0.4 * _v, 600, panX); }
            },
            // NEW: Biome reveal stinger
            biomeReveal() { [262, 330, 392, 523, 659].forEach((f, i) => { ctx.scheduleSfx(() => beep('sine', f, f, 0.18, 0.22), i * 90); }); ctx.scheduleSfx(() => { beep('triangle', 523, 523, 0.4, 0.2); beep('sine', 784, 784, 0.4, 0.12); }, 500); },
            // NEW: Bonus sub-stage jingle
            bonusStart() { [523, 659, 784, 659, 784, 988].forEach((f, i) => { ctx.scheduleSfx(() => beep('square', f, f, 0.1, 0.2), i * 70); }); },
            bonusEnd(rating) { const base = rating === 'S' ? [784, 988, 1175, 1568] : rating === 'A' ? [659, 784, 988, 1319] : [523, 659, 784, 1047]; base.forEach((f, i) => { ctx.scheduleSfx(() => beep('sine', f, f, 0.12, 0.25), i * 80); }); },
            // NEW: UI sounds
            ...
            shopBuy() { beep('sine', 880, 1200, 0.08, 0.2); ctx.scheduleSfx(() => beep('sine', 1320, 1320, 0.08, 0.2), 80); },
            shopError() { beep('sawtooth', 200, 120, 0.12, 0.2); },
            achievementUnlock() { [523, 659, 784, 1047].forEach((f, i) => { ctx.scheduleSfx(() => beep('sine', f, f, 0.15, 0.28), i * 70); }); },
            // NEW: Whoosh for dives/parries/supers — noise + bandpass sweep
            whoosh(speed, panX) { noise(Math.max(0.08, Math.min(0.3, 60 / Math.max(50, speed))), 0.18, 2200, panX); },
            ...
            superNovaBarrage(panX) { const _v = vv(); beep('sawtooth', 200 * pv(), 1400, 0.3, 0.45 * _v, panX); beep('square', 100, 300, 0.25, 0.25 * _v, panX); ctx.scheduleSfx(() => noise(0.4, 0.5 * _v, 800, panX), 250); },
            superPhaseDash(panX) { const _v = vv(); beep('sine', 300, 3000, 0.3, 0.4 * _v, panX); beep('sawtooth', 150, 2000, 0.25, 0.25 * _v, panX); ctx.scheduleSfx(() => noise(0.15, 0.3 * _v, 5000, panX), 200); },
            ...
            superShadowClone(panX) { const _v = vv(); beep('sine', 600, 1800, 0.35, 0.35 * _v, panX); beep('triangle', 900, 2400, 0.3, 0.25 * _v, panX); ctx.scheduleSfx(() => { beep('sine', 1200, 1200, 0.2, 0.2 * _v, panX); beep('sine', 1500, 1500, 0.2, 0.15 * _v, panX); beep('sine', 1800, 1800, 0.2, 0.12 * _v, panX); }, 150); },
            // NEW: Stage archetype cues
            archetypeSwarmLoop() { const _v = vv(); beep('square', 100, 100, 0.1, 0.18 * _v); ctx.scheduleSfx(() => beep('square', 100, 100, 0.1, 0.18 * _v), 500); },
            ...
            riskItToggle() { beep('square', 600, 900, 0.05, 0.18); ctx.scheduleSfx(() => beep('triangle', 900, 1200, 0.05, 0.18), 50); },
            // NEW: Rank jingles
            rankJingleSplus() { const _v = vv(); [784, 988, 1175, 1568, 2093, 2637].forEach((f, i) => { ctx.scheduleSfx(() => beep('sine', f, f, 0.12, 0.3 * _v), i * 60); }); },
            rankJingleS() { const _v = vv(); [659, 784, 988, 1319, 1568].forEach((f, i) => { ctx.scheduleSfx(() => beep('triangle', f, f, 0.1, 0.25 * _v), i * 70); }); },
            rankJingleA() { const _v = vv(); [523, 659, 784, 1047].forEach((f, i) => { ctx.scheduleSfx(() => beep('sine', f, f, 0.1, 0.22 * _v), i * 80); }); },
            rankJingleB() { beep('triangle', 440, 440, 0.15, 0.2); ctx.scheduleSfx(() => beep('triangle', 660, 660, 0.15, 0.2), 150); },
            rankJingleC() { const _v = vv(); beep('sawtooth', 220, 110, 0.4, 0.25 * _v); beep('triangle', 165, 82, 0.3, 0.2 * _v); },
            // NEW: Boss phase stingers
            bossPhaseTransition() { const _v = vv(); beep('sawtooth', 100, 50, 0.6, 0.4 * _v); noise(0.4, 0.3 * _v, 800); ctx.scheduleSfx(() => beep('sawtooth', 200, 100, 0.3, 0.25 * _v), 200); },
            ...
            bossDeathStinger() { const _v = vv(); [110, 82, 65, 49, 41].forEach((f, i) => { ctx.scheduleSfx(() => { beep('sawtooth', f, f * 0.5, 0.6, 0.35 * _v); beep('sine', f * 2, f, 0.5, 0.25 * _v); }, i * 400); }); },
            ...
            powerupChime(panX) { [660, 990, 1320].forEach((f, i) => ctx.scheduleSfx(() => ctx.fm(f, f, 0.22, 0.22, panX, 3), i * 65)); },
            warpWhoosh() { noise(0.35, 0.24, 1200); ctx.fm(90, 900, 0.32, 0.2); },
            comboRiser(level, panX) { const base = 440 * Math.pow(1.1892, Math.min(level, 8)); [1, 1.26, 1.5, 2, 2.52].forEach((iv, i) => { ctx.scheduleSfx(() => beep('sine', base * iv, base * iv, 0.09, (0.2 + level * 0.02) * vv(), panX), i * 45); }); },
            megaComboStinger(level, panX) { if (ctx.G.muted) return;
                ...
                [0, 4, 7, 12].forEach((semi, i) => {
                    ...
                    ctx.scheduleSfx(() => beep('square', f, f, 0.1, 0.22 + i * 0.03, panX), 220 + i * 55);
                });
            },
            // NEW: Short high sparkle twinkle for precision score bonuses (headshots)
            sparkleTwinkle(panX) { const _p = pv(); beep('sine', 2400 * _p, 2400 * _p, 0.05, 0.16, panX); ctx.scheduleSfx(() => beep('sine', 3600 * _p, 3600 * _p, 0.06, 0.14, panX), 55); },
            ...
            bossEntrance(panX) { const _p = pv(), _v = vv(); beep('sawtooth', 60 * _p, 30 * _p, 0.8, 0.45 * _v, panX); beep('triangle', 40 * _p, 20 * _p, 0.6, 0.3 * _v, panX); noise(0.6, 0.25 * _v, 120, panX); ctx.scheduleSfx(() => { beep('sawtooth', 120 * _p, 80 * _p, 0.3, 0.3 * _v, panX); noise(0.2, 0.15 * _v, 300, panX); }, 300); },
            // NEW: Stage clear fanfare — triumphant ascending arpeggio with reverb tail
            stageFanfare(panX) {
                const _v = vv();
                [523, 659, 784, 1047, 1319, 1568, 2093, 2637].forEach((f, i) => {
                    ctx.scheduleSfx(() => { beep('triangle', f, f, 0.12, 0.25 * _v, panX); if (i >= 4) beep('sine', f * 0.5, f * 0.5, 0.1, 0.1 * _v, panX); }, i * 60);
                });
                ctx.scheduleSfx(() => { beep('sine', 1047, 1047, 0.4, 0.2 * _v, panX); beep('triangle', 1568, 1568, 0.4, 0.15 * _v, panX); }, 560);
            },
            stageClearFanfare(panX) { if (ctx.G.muted) return;
                ...
                melody.forEach((f, i) => ctx.scheduleSfx(() => {
                    if (ctx.G.muted) return;
            ...
            playerDeathWhoosh(panX) { noise(0.4, 0.35, 1600, panX); ctx.fm(180, 35, 0.4, 0.3, panX); },
            ...
            bossDeathRumble(panX) {
                ...
                ctx.scheduleSfx(() => { noise(0.4, 0.15 * _v, 200, panX); beep('sawtooth', 50 * _p, 25 * _p, 0.4, 0.2 * _v, panX); }, 400);
                ctx.scheduleSfx(() => { noise(0.2, 0.08 * _v, 300, panX); }, 800);
            },
            screenShatter() { if (ctx.G.muted) return; const _v = vv(); noise(0.25, 0.35 * _v, 5000); [880, 660, 440].forEach((f, i) => ctx.scheduleSfx(() => beep('sawtooth', f, f * 0.5, 0.08, 0.22 * _v), i * 40)); },
            ...
            rankSlam() { if (ctx.G.muted) return; const _v = vv(); beep('square', 220, 220, 0.04, 0.34 * _v); [523, 659, 784, 1047].forEach((f, i) => ctx.scheduleSfx(() => beep('square', f, f * 0.98, 0.055, (0.36 - i * 0.02) * _v), i * 32)); },
            hyperShift() { if (ctx.G.muted) return; beep('sawtooth', 180, 720, 0.2, 0.3 * vv()); ctx.scheduleSfx(() => beep('sine', 360, 1080, 0.15, 0.22 * vv()), 80); },
            mirrorPing(panX) { if (ctx.G.muted) return; beep('sine', 1600, 2200, 0.05, 0.16 * vv(), panX); beep('triangle', 1200, 800, 0.06, 0.12 * vv(), panX); },
            modeSelect() { if (ctx.G.muted) return; beep('square', 660, 990, 0.05, 0.2 * vv()); ctx.scheduleSfx(() => beep('triangle', 990, 1320, 0.05, 0.18 * vv()), 60); },
            gauntletWave() { if (ctx.G.muted) return; [523, 659, 784].forEach((f, i) => ctx.scheduleSfx(() => beep('sawtooth', f, f, 0.08, 0.24 * vv()), i * 70)); },
            ...
            megabomb(panX) { if (ctx.G.muted) return; noise(0.75, 0.75 * vv(), 1400, panX); beep('sawtooth', 80, 25, 0.55, 0.55 * vv(), panX); ctx.scheduleSfx(() => beep('sine', 40, 20, 0.4, 0.35 * vv(), panX), 120); },
            ...
            superReady(panX) { if (ctx.G.muted) return;
                ...
                ctx.scheduleSfx(() => { if (ctx.G.muted) return; beep('sine', 1760, 2200, 0.12, 0.16 * _v, panX); }, 90);
                ctx.scheduleSfx(() => ctx.fm(2637 * _p, 2637 * _p, 0.35, 0.16 * _v, panX, 3), 160);
            },
            // NEW: Last-life heartbeat — two soft low thumps on a slow cadence
            heartbeat() { if (ctx.G.muted) return;
                const _v = vv();
                beep('sine', 70, 45, 0.13, 0.18 * _v);
                ctx.scheduleSfx(() => { if (ctx.G.muted) return; beep('sine', 62, 42, 0.11, 0.12 * _v); }, 150);
            },
            ...
            multiKill(panX) { if (ctx.G.muted) return;
                ...
                ctx.scheduleSfx(() => { if (ctx.G.muted) return; beep('square', 392 * _p, 588 * _p, 0.14, 0.24 * _v, panX); beep('triangle', 588 * _p, 784 * _p, 0.12, 0.14 * _v, panX); }, 70);
            }
        };
        SFX.attackWarning = pan => ctx.fm(680, 900, 0.10, 0.18, pan, 1.5);
        SFX.sectorAttack = (sector, pan) => { const i = GC.SECTORS.findIndex(s => s.id === sector); ctx.fm(180 + i * 40, 60 + i * 20, 0.18, 0.27, pan, 2 + i / 2); noise(0.06, 0.1, 1600, pan); };
        SFX.waveArrive = () => beep('triangle', 330, 495, 0.12, 0.10);
        for (const [name, play] of Object.entries(SFX)) {
            SFX[name] = (...args) => {
                const previous = ctx.sfxPriority;
                ctx.sfxPriority = /Warning|parry|shield|pExplode|respawn|Ready/.test(name) ? 3 : /shoot|Shoot|rocketLaunch|weaponArm/.test(name) ? 2 : 1;
                try { return play.apply(SFX, args); } finally { ctx.sfxPriority = previous; }
            };
        }
        ctx.SFX = SFX;
    };
