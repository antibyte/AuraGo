(function () {
    ...
    function detailPane(state) {
        ...
        return `<article class="vc-machine-detail">
            ...
            <dl class="vc-meta-grid">
                ...
                <div><dt>${esc(tx(c, 'desktop.virtual_computers_runtime'))}</dt><dd>${esc(formatDuration(machine.ttl_seconds))}</dd></div>
    ...
    async function launch(state) {
        ...
        const body = {
            ...
            ttl_seconds: Number(state.host.querySelector('[data-role="ttl"]')?.value || 600)
        };
