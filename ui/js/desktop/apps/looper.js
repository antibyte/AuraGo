(function () {
    ...
    function render(container, windowId, context) {
        ...
        container.innerHTML = `
            <div class="vd-looper">
                ...
                <div class="vd-looper-monitor" id="looper-monitor-${windowId}">
                    ...
                    <div class="vd-looper-current-step" id="looper-current-step-${windowId}">
                        <div class="vd-looper-current-step-label"></div>
                        <div class="vd-looper-current-step-text"></div>
                    </div>
        ...
        function updateStatus(data) {
            ...
            // Highlight active step & show current prompt in monitor
            $$(`.vd-looper-step`).forEach(stepEl => {
                stepEl.classList.remove('vd-looper-step--active');
            });
            const currentStepEl = $(`looper-current-step-${windowId}`);
            if (data.running && data.current_step && data.current_step !== 'idle') {
                ...
                const textarea = $(`looper-${data.current_step}-${windowId}`);
                const stepMeta = STEP_META.find(s => s.key === data.current_step);
                if (textarea && textarea.value && stepMeta) {
                    const labelEl = currentStepEl.querySelector('.vd-looper-current-step-label');
                    const textEl = currentStepEl.querySelector('.vd-looper-current-step-text');
                    if (labelEl) labelEl.textContent = esc(t(stepMeta.labelKey));
