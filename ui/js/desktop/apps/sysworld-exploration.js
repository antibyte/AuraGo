...
export const interiors = [
  {id:'agent',x:0,z:14,width:12,depth:12,doorZ:20,liftX:4,liftZ:11},
  {id:'memory',x:-24,z:12,width:12,depth:12,doorZ:18,liftX:-20,liftZ:9},
  {id:'missions',x:43,z:62,width:12,depth:12,doorZ:68,liftX:47,liftZ:59},
];
export const stations = [
  {id:'infra',x:-74,z:-53},{id:'memory',x:-74,z:-10},{id:'graph',x:-43,z:73},
  {id:'operations',x:0,z:73},{id:'missions',x:43,z:73},{id:'integrations',x:74,z:-10},{id:'agent',x:0,z:25},
];
export const tramWaypoints=[[-74,-53],[-74,-10],[-74,73],[-43,73],[0,73],[43,73],[74,73],[74,-10],[74,-83],[18,-83],[18,25],[0,25],[-16,25],[-16,-83],[-74,-83]];
export const dronePad={x:58,z:14};
...
function solidBuilding(x,z,districts){
  ...
  return [[29,-55],[49,-55],[-3,-57],[4,-73]].some(([tx,tz])=>Math.abs(x-tx)<6&&Math.abs(z-tz)<5);
}
...
export function groundHeight(x,z){
  ...
  if(x>=-82&&x<=-75&&z>=46&&z<=52)return (52-z)/3;
  if(x>=-82&&x<=-75&&z>=34&&z<46)return 2;
  if(x>=-82&&x<=-75&&z>=28&&z<34)return (z-28)/3;
  return 0;
}
export function canWalk(x,z,from,doorOpen,districts){
  ...
  if(next||prev){
    ...
    if(next?.id!==prev?.id){
      ...
      if(Math.abs(x-r.x)>1.15||Math.abs(from.x-r.x)>1.15||Math.min(z,from.z)<r.doorZ-.9||!doorOpen(r.id))return false;
    }
    if(next && (Math.abs(x-r.x)>5.55||z<r.z-5.55))return false;
    return true;
  }
