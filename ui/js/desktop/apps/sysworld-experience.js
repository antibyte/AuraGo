...
export function createExperience(scene,options){
  ...
  const staticGroups=new Map(),loaded=new Set(),bindings=[],textures=new Set(),freight=[],missionStates=new Map(),controller=new AbortController();
  ...
  const route=new THREE.CatmullRomCurve3(tramWaypoints.map(([x,z])=>new THREE.Vector3(x,.15,z)),true,'catmullrom',.08),length=route.getLength();
  const walkRoute=new THREE.CatmullRomCurve3([[-80,-80],[-80,65],[-70,71],[70,71],[80,65],[80,-80],[70,-87],[-70,-87]].map(([x,z])=>new THREE.Vector3(x,0,z)),true,'catmullrom',.02),walkLength=walkRoute.getLength();
  ...
  async function place(id,x,z,y=0,angle=0,parent=group){
    const gltf=await template(id);if(disposed)return null;
    const node=gltf.scene.clone(true);node.position.set(x,y,z);node.rotation.y=angle;parent.add(node);
  ...
  async function loadRoom(r){
    if(rooms.has(r.id)||disposed)return;const shell=new THREE.Group();group.add(shell);rooms.set(r.id,{shell,ready:false,lift:null,liftValue:0,liftTarget:0});loading++;
    try{
      const tasks=[];
      for(let x=-4;x<=4;x+=4)for(let z=-4;z<=4;z+=4){tasks.push(place('floor',r.x+x,r.z+z,0,0,shell));tasks.push(place('ceiling',r.x+x,r.z+z,8.15,0,shell));}
      for(let x=-4;x<=4;x+=4)tasks.push(place('wall',r.x+x,r.z-6,0,0,shell));
      for(let z=-4;z<=4;z+=4)for(const side of[-1,1])tasks.push(place('window',r.x+side*6,r.z+z,0,Math.PI/2,shell));
      for(let z=-4;z<=4;z+=4)for(const side of[-1,1])tasks.push(place('window',r.x+side*6,r.z+z,4,Math.PI/2,shell));
      for(let x=-4;x<=4;x+=4)for(const side of[-1,1])tasks.push(place('window',r.x+x,r.z+side*6,4,0,shell));
      for(const x of[-4,4])tasks.push(place('wall',r.x+x,r.doorZ,0,0,shell));
      for(const x of[-3,0])tasks.push(place(r.id==='missions'?'cargo':r.id==='memory'?'archive-shelf':'console',r.x+x,r.z-3,0,0,shell));
      tasks.push(place('bench',r.x-4,r.z+1,0,Math.PI/2,shell));
      await Promise.all(tasks);if(disposed)return;
      const info=rooms.get(r.id);info.shell=batch(shell);
      info.hologram=await place('hologram',r.x-1,r.z,0);info.hologram?.play('operate');
      if(r.id==='memory')drawMemory();
      info.lift=await place('lift',r.liftX,r.liftZ,0);info.liftAction=info.lift?.play('operate',true);if(info.liftAction)info.liftAction.paused=true;
      // Accessible balcony attached to the lift's top landing.
      const balcony=new THREE.Group();group.add(balcony);await place('floor',r.x+2,r.z-3,4,0,balcony);info.balcony=batch(balcony);
      info.ready=true;
  ...
  async function init(){
    try{
      ...
      for(const s of stations)tasks.push(place('station',s.x+(Math.abs(s.x)>70?-5:0),s.z+(Math.abs(s.x)>70?0:4),0,Math.abs(s.x)>70?Math.PI/2:0,outdoors));
      for(const r of interiors){const d=await place('door',r.x,r.doorZ);if(disposed)return;const a=d.play('open',true);a.paused=true;doors.set(r.id,{...d,action:a,value:0,open:false});}
      for(let x=-76;x<=76;x+=8)tasks.push(place('quay',x,77,0,0,outdoors));
      for(const [x,z]of[[-78,61],[-78,-67],[57,24],[9,36],[55,-27]]){tasks.push(place('garden',x,z,0,0,outdoors));tasks.push(place('bench',x+3,z,0,Math.PI/2,outdoors));}
      for(const [x,z]of[[-30,19],[30,18],[-12,65]])tasks.push(place('arcade',x,z,0,0,outdoors));
      tasks.push(place('bridge',-78.5,40,2,0,outdoors),place('ramp',-80,49,0,Math.PI,outdoors),place('stairs',-76.8,49,0,Math.PI,outdoors),place('ramp',-78.5,31,0,0,outdoors));
      tasks.push(place('pad',dronePad.x,dronePad.z,0,0,outdoors));
      for(const [x,z]of[[-64,-56],[-62,-46],[51,45]])tasks.push(place('charger',x,z,0,0,outdoors));
      ...
      const cart=await place('service-cart',-63,60);if(cart){cart.node.rotation.y=Math.PI/2;cart.play('open',true);}
      for(let i=0;i<3;i++){const box=await place('cargo',43,61);if(box){box.node.visible=false;freight.push({...box,elapsed:9,direction:1});}}
      options.onReady?.();
    }catch(e){if(!disposed)options.onError?.(e);}
  }
  void init();
  function drawMemory(){
    const info=rooms.get('memory');if(!info?.hologram)return;
    if(!info.text){const canvas=document.createElement('canvas');canvas.width=1024;canvas.height=512;const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;textures.add(texture);
      ...
      const mesh=new THREE.Mesh(geometry,material);mesh.position.set(-25,3.6,11.7);group.add(mesh);info.text={canvas,texture,mesh};}
  ...
  function interact(){
    ...
    if(near.kind==='lift'){const r=rooms.get(near.id);if(r){r.liftTarget=r.liftTarget?0:4;ride={kind:'lift',id:near.id};options.onRide?.('lift');callSound('lift',camera.position);}}
  }
  function endRide(){
    if(!ride)return;
    if(ride.kind==='tram'){const s=stations.find(s=>s.id===ride.station)||stations[0];camera.position.set(s.x+4,2.4,s.z+2);}
  ...
  function candidates(){
    ...
    for(const r of interiors){add('door',r.id,r.x,r.doorZ,3);if(rooms.get(r.id)?.ready){add('lift',r.id,r.liftX,r.liftZ,2.3);add('terminal',r.id,r.x-2,r.z-3,2.5);}}
    for(const s of stations){add('tram',s.id,s.x,s.z,6);if(!visited.has(s.id))add('discover',s.id,s.x+5,s.z+3,4);}
  ...
  function update(dt,animated,mode){
    ...
    for(const [id,r]of rooms){if(!r.liftAction)continue;r.liftValue=THREE.MathUtils.damp(r.liftValue,r.liftTarget,1.8,Math.min(dt,.1));r.liftAction.time=r.liftValue/4*r.liftAction.getClip().duration;r.lift.mixer.update(0);if(ride?.kind==='lift'&&ride.id===id){const pos=interiors.find(i=>i.id===id);camera.position.set(pos.liftX,2.4+r.liftValue,pos.liftZ);if(Math.abs(r.liftValue-r.liftTarget)<.02){ride=null;options.onRide?.(null);}}}
    ...
    trams.forEach((t,i)=>{
      ...
      route.getPointAt(t.u,t.node.position);route.getTangentAt(t.u,ahead);t.node.rotation.y=Math.atan2(ahead.x,ahead.z);
      ...
      if(ride?.kind==='tram'&&ride.index===i){if(t.dwell>1){ride.station=stops[t.stop].id;if(ride.exitRequested){endRide();return;}}camera.position.copy(t.node.position).addScaledVector(ahead,ride.offset);camera.position.y+=2.4;camera.lookAt(t.node.position.x+ahead.x*16,2.5,t.node.position.z+ahead.z*16);}
    });
    if(ride?.kind==='drone'){
    ...
    for(const box of freight){box.elapsed+=step;box.node.visible=animated&&box.elapsed<8;if(!box.node.visible)continue;const t=box.direction>0?box.elapsed/8:1-box.elapsed/8;box.node.position.set(31+t*12,.15,62);}
  ...
  return {update,interact,endRide,setTier,
    ...
    move(x,z,from){if(ride)return false;const r=roomAt(from.x,from.z);if(r&&from.y>4){const balcony=x>r.x+.35&&x<r.x+4&&z>r.z-4.6&&z<r.z-1.4,lift=rooms.get(r.id)?.liftValue>3.9&&Math.abs(x-r.liftX)<1.1&&Math.abs(z-r.liftZ)<1.1;return balcony||lift;}
      if(r&&from.y<4&&(Math.hypot(x-(r.x-1),z-r.z)<1.55||[-3,0].some(dx=>Math.abs(x-r.x-dx)<1.1&&Math.abs(z-r.z+3)<.8)))return false;
      return canWalk(x,z,from,id=>doors.get(id)?.value>.85&&!!rooms.get(id)?.ready,options.districts);},
    floor(x,z){if(roomAt(x,z)&&camera.position.y>4)return 4;return Math.max(groundHeight(x,z),buildingFloor(x,z,options.districts));},
    visit(id){endRide();if(id==='drone'){camera.position.set(dronePad.x,2.4,dronePad.z+3);camera.lookAt(dronePad.x,2,dronePad.z);return;}const s=stations.find(s=>s.id===id);if(s){camera.position.set(s.x+5,2.4,s.z+3);camera.lookAt(s.x,2,s.z);}},
    destination(id){const r=interiors.find(r=>r.id===id);if(r){endRide();camera.position.set(r.x,2.4,r.doorZ+4);camera.lookAt(r.x,2,r.z);}},
