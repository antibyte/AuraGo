(function () {
    ...
    const WIDGET_AUTO_SIZE_PADDING = 6, WIDGET_FRAME_SCROLLBAR_BUFFER = 24, WIDGET_FRAME_CHROME_BUFFER = 32;
    ...
    function applyWidgetAutoSize(card, payload) { if (!card || card.dataset.widgetAutoSize !== 'true') return; const data = payload && typeof payload === 'object' ? payload : {}; const frameWrap = card.querySelector('.vd-widget-frame-wrap'); const reportedFrameHeight = Number(data.height || data.h || 0); if (frameWrap && reportedFrameHeight > 0) { const frameHeight = clampWidgetFrameHeight(card, reportedFrameHeight + WIDGET_FRAME_SCROLLBAR_BUFFER); card.style.setProperty('--vd-widget-frame-height', frameHeight + 'px'); frameWrap.style.setProperty('--vd-widget-frame-height', frameHeight + 'px'); } const measuredContentHeight = widgetMeasuredContentHeight(card, data); const desiredHeight = Math.max(WIDGET_MIN_HEIGHT, Math.ceil(Number(data.cardHeight || data.card_height || 0)), measuredContentHeight, Math.ceil(card.scrollHeight || 0)); card.style.setProperty('--vd-widget-auto-height', clampWidgetHeight(card, desiredHeight, WIDGET_MIN_HEIGHT) + 'px'); }
    function resizeWidgetToContent(widgetId, payload) { const id = String(widgetId || ''); if (!id) return; const card = document.querySelector(`.vd-widget[data-widget-id="${cssSel(id)}"]`); if (!card || card.dataset.widgetAutoSize !== 'true') return; const data = payload && typeof payload === 'object' ? payload : {}; const reportedWidth = Number(data.width || data.w || 0); if (reportedWidth > 16) { const nextWidth = Math.max(220, Math.min(Math.ceil(reportedWidth + WIDGET_FRAME_CHROME_BUFFER), widgetMaxWidth(card))); card.style.width = nextWidth + 'px'; } applyWidgetAutoSize(card, data); }
