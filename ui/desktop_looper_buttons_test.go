...
func TestDesktopLooperStylesBustComponentCache(t *testing.T) {
	...
	if !strings.Contains(desktopCSS, "@import url('desktop-apps.css?v=20260524-audit-fixes');") {
	...
	if !strings.Contains(desktopHTML, `/css/desktop.css?v={{.BuildVersion}}-desktop-20260524-audit-fixes`) {
