...
home_assistant:
  ...
  allowed_services: ["light.turn_on", "light.turn_off"]  # Empty allows all services
...
- **Service allowlist**: `home_assistant.allowed_services` is optional. Empty allows all service calls unless blocked by `home_assistant.blocked_services`; non-empty restricts `call_service` to listed `domain.service` entries.
