...
- Use `AuraDesktop.fs`, `AuraDesktop.widgets.register`, `AuraDesktop.widgets.resize`, `AuraDesktop.notifications.show`, and `AuraDesktop.desktop.openApp` for desktop actions. The SDK talks to the desktop shell through a safe iframe bridge. Widget iframes auto-report content size when the SDK is loaded; call `AuraDesktop.widgets.resize()` after large async layout changes if the widget content changes outside normal DOM resize observation.
