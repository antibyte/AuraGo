...
- **`operation`**: (Required) The action to perform. Must be one of:
  ...
  - `"run_command"`: Dispatches a command string to be executed by the remote MeshAgent.
...
- **`node_id`**: (Required for `wake`, `power_action`, `run_command`) The unique identifier for a specific device endpoint.
...
- **`command`**: (Required for `run_command`) The command string to execute on the remote device's shell.
