...
Binary assets are supported and managed through the upload/download API. File management API endpoints include: `POST /api/agent-skills/{id}/files` (JSON `path`/`content`) to create or update text files, `POST /api/agent-skills/{id}/files/upload` (multipart, `path` form field) to upload binary assets, `GET /api/agent-skills/{id}/files/raw?path=...` to download raw bytes, `POST /api/agent-skills/{id}/files/rename` (JSON `from`/`to`) to rename files, and `DELETE /api/agent-skills/{id}/files?path=...` to remove files. The path is always a query parameter or JSON field, never a URL path segment. The `POST /api/agent-skills` endpoint also accepts a `resources` field for create-with-resources.
