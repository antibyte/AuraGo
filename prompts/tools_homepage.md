...
**CRITICAL — File management:** Always use `homepage` → `write_file` / `read_file` / `list_files` for all homepage project files. The generic `filesystem` tool writes to `agent_workspace/workdir/`, not the homepage workspace (`data/homepage/`), so files created there will not be found by build, deploy, or publish operations. If `init_project` fails, use `homepage` → `write_file` to create files manually in the correct location.
