...
1. **Check Memory.** Before analyzing the error, search your memory for past solutions. Run `query_memory` across `error_patterns`, `journal`, and `cheatsheets` with a query describing the error or failing component. If you have resolved this before, reuse or adapt the documented solution — do not start from scratch.
