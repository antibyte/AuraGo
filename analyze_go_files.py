#!/usr/bin/env python3
"""
Analyse aller Go-Dateien für KI-Verarbeitungsoptimierung
"""
...
# Kategorien für KI-Verarbeitung
CATEGORIES = {
    'ideal': (0, 300, '✅ Ideal', 'Perfekt für KI-Verarbeitung'),
    'good': (301, 500, '✅ Gut', 'Noch gut handhabbar'),
    'acceptable': (501, 800, '⚠️ Akzeptabel', 'Groß, aber noch OK'),
    'large': (801, 1200, '⚠️ Groß', 'Sollte refactored werden'),
    'too_large': (1201, 2000, '🔴 Zu groß', 'Aufspaltung empfohlen'),
    'critical': (2001, float('inf'), '🔴 Kritisch', 'Sofortige Aufspaltung nötig'),
}
...
def generate_report():
    ...
    report.append(f"  Gesamtgröße: {total_size:.2f} KB")
    report.append("")
    ...
    report.append("-" * 80)
    report.append("TOP 20 GRÖßTE DATEIEN:")
    report.append("-" * 80)
    report.append("")
    report.append(f"{'Datei':<55} {'Zeilen':>8} {'KB':>8} {'Status':<15}")
    report.append("-" * 80)
    ...
    for gf in all_files[:20]:
        ...
        report.append(f"{filename:<55} {gf.lines:>8} {gf.size_kb:>8.1f} {status:<15}")
    
    # Aufteilungsempfehlungen
    report.append("")
    report.append("=" * 80)
    report.append("AUSSPLITUNGSEMPFEHLUNGEN")
    report.append("=" * 80)
    report.append("")
    ...
    # Prioritäten
    report.append("-" * 80)
    report.append("PRIORITÄTEN FÜR REFACTORING:")
    report.append("-" * 80)
    report.append("")
    ...
    report.append("")
    report.append("2. HOCH (Zu groß - 1200-2000 Zeilen):")
    ...
    report.append("")
    report.append("3. MITTEL (Groß - 800-1200 Zeilen):")
    large = files_by_category.get('large', [])
    if large:
        for gf in large[:10]:
            report.append(f"   ⚠️ {gf.relative_path} ({gf.lines} Zeilen)")
        if len(large) > 10:
            report.append(f"   ... und {len(large) - 10} weitere")
    
    report.append("")
    report.append("=" * 80)
    report.append("ZIEL: Alle Dateien unter 800 Zeilen für optimale KI-Verarbeitung")
    report.append("=" * 80)
...
if __name__ == "__main__":
    ...
    # Speichern
    with open("go_files_analysis_report.txt", "w", encoding="utf-8") as f:
        f.write(report)
    
    # Ausgabe
    print(report)
    print(f"\nBericht gespeichert in: go_files_analysis_report.txt")
