    ...
    let items: Vec<ListItem> = if app.skills.is_empty() {
        ...
        app.skills
            ...
            .map(|(i, s)| {
                ...
                let name = utils::truncate_str(&s.name, 18);
                ListItem::new(Line::from(vec![
                    Span::styled(
                        format!("{}{}{} ", sec_icon, daemon_icon, enabled_icon),
                        style,
                    ),
    ...
    let text = if let Some(s) = skill {
        let mut lines = vec![
            ...
            Line::from(vec![
                ...
                Span::styled(
                    format!(
                        "{} {}",
                        security_icon(&s.security_status),
                        s.security_status
                    ),
        ...
        if !s.description.is_empty() {
            ...
            for line in desc_lines {
                let truncated = format!("  {}", utils::truncate_str(line, 100));
                lines.push(Line::from(Span::styled(
                    truncated,
                    Style::default().fg(theme.fg),
                )));
